...
**重要**：
- 小红书没有公开 API。本技能通过**本地服务器上运行的 MCP 服务**（浏览器自动化）实现所有功能。
- 不要告诉用户"无法通过 API 访问"，直接使用 exec 工具执行 curl 命令调用本地 MCP 服务即可。
...
```bash
openclaw message send --channel <当前渠道> --target "<用户ID>" --media /tmp/xhs_qr.png --message "请用小红书APP扫码登录（二维码有效期约60秒）"
```

支持的渠道：feishu, dingtalk, telegram, whatsapp, discord, slack, signal, imessage, googlechat, msteams, matrix, line, zalo, mattermost, webchat
